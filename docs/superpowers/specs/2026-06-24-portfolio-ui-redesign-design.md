...
> **范围**: 门户外壳(React/Vite/Tailwind)视觉升级 + 组件体系 + 加载反馈 + 页面级更新公告 + 主题切换器(浅色/深蓝/赛博绿)。
> **本轮不做**: iframe demo 内部换肤(放第二轮)。
> **主题默认**: 浅色现代风,用户可切换并持久化到 localStorage。
> **日期**: 2026-06-24
...
- 建立可复用的 UI 组件与 CSS 设计令牌,支持浅色/深蓝/赛博绿三套主题切换。
...
- 主题切换器在导航栏可用,浅色/深蓝/赛博绿三套主题切换正常,刷新后保持选择。
...
4. **主题系统**: 所有颜色走 CSS 变量,默认浅色,同时预置深蓝与赛博绿两套主题属性,通过 `data-theme` 属性切换。主题选择持久化到 `localStorage`。  

---

## 3. 设计令牌(CSS 变量)

所有颜色、间距、圆角、阴影统一用 CSS 变量定义,通过 `data-theme` 属性切换浅色(light)、深蓝(deepblue)、赛博绿(cyber)三套主题。
...
### 3.2 深蓝科技主题 `[data-theme="deepblue"]`
...
Tailwind 通过 `tailwind.config.js` 的 `extend.colors` 把这些变量映射成 Tailwind 类名(如 `bg-surface` 等),避免手写大量内联样式。主题切换通过修改 `<html>` 标签的 `data-theme` 属性实现。
...
- 右侧:首页 / AI 作品 / 学习 / 个人四个链接 + **主题切换器 `ThemeToggle`**。
- 主题切换器:按钮组形式,三个选项“浅色 / 深蓝 / 赛博”。点击后即时切换 `data-theme`,保存到 `localStorage`。
- 当前页链接高亮(字重 600,颜色 `--text-primary`),其余 `--text-tertiary`。
- 移动端:链接收进汉堡菜单(用 React state + 简单展开面板,不引入新依赖),主题切换器保留在菜单内。
...
| `ThemeToggle` | `components/ThemeToggle.tsx` | 导航栏主题切换按钮组 |
| `useTheme` | `hooks/useTheme.ts` | 读取/切换/持久化主题 |
...
- `frontends/portfolio/src/components/ThemeToggle.tsx`
- `frontends/portfolio/src/hooks/useTheme.ts`
- `frontends/portfolio/src/hooks/useDocumentTitle.ts`
- `frontends/portfolio/src/hooks/useChangelog.ts`
